    ...
    public class ImponerEncomiendaCallCenterModelo
    {
        ...
        */
        ...
        )
        {
            ...
            // Cliente
            var cli = _clientes.FirstOrDefault(c => Digits(c.Cuit) == cuitDigits);
            if (cli is null)
            { MessageBox.Show("CUIT inexistente.", "Validación"); return creadas; }

            // CD de origen por provincia del cliente
            int cpCliente = ClienteAlmacen.clientes.FirstOrDefault(e => Digits(e.CUIT) == cuitDigits)?.CodigoPostal ?? 0;
            int cpCDOrigen = CentroDeDistribucionAlmacen.centrosDeDistribucion
                .Where(cd => LocalidadAlmacen.localidades.Any(lcd => lcd.CodigoPostal == cd.CodigoPostal &&
                            LocalidadAlmacen.localidades.FirstOrDefault(l => l.CodigoPostal == cpCliente)?.Provincia.Equals(lcd.Provincia) == true))
                .Select(cd => (int?)cd.CodigoPostal)
                .FirstOrDefault() ?? 0;

            if (cpCDOrigen == 0 && _cdsPorProv.TryGetValue(provinciaId, out var cdsProv) && cdsProv.Count > 0)
            { cpCDOrigen = cdsProv[0].id; }
